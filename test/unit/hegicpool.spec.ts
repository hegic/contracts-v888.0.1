...
describe("HegicPool", async () => {
  ...
  describe("constructor & settings", async () => {
    it("should set all initial state", async () => {
      ...
      expect(await hegicPool.HEDGE_FEE_RATE()).to.be.eq(BN.from(80))
  ...
  describe("lock", async () => {
    ...
    it("should revert if the caller is not the admin", async () => {
      await expect(
        hegicPool.connect(alice).lock(BN.from(1), BN.from(1), BN.from(0)),
    ...
    it("should revert if the locked amount is too large", async () => {
      await expect(
        ...
        hegicPool.lock(BN.from(90000), BN.from(1), BN.from(0)),
    ...
    it("should create locked liquidity", async () => {
      ...
      // 10 * 10000 / 10000
      await hegicPool.lock(BN.from(10000), BN.from(0), BN.from(0))
    ...
    it("should transfer the hedge fee to the hedge pool", async () => {
      ...
      await fakeWBTC.transfer(hegicPool.address, premium)
      await hegicPool.lock(BN.from(10000), premium, BN.from(0))

      const poolTotalBalance = await hegicPool.totalBalance()
      const poolHedgedBalance = await hegicPool.hedgedBalance()
      const poolHedgeFeeRate = await hegicPool.HEDGE_FEE_RATE()
